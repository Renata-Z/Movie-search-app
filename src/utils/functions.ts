...
export function createDropdownMenu(input: HTMLElement, optionsArr: MovieList[]) {
  const dropdownMenuWrapper = document.createElement('div');
  const item = document.querySelector('autocomplete-item');
  dropdownMenuWrapper.setAttribute("class", "autocomplete-list");
  // dropdownMenuWrapper.setAttribute("class", "autocomplete-items");
  input.parentNode?.appendChild(dropdownMenuWrapper);
  
  optionsArr.forEach(movie => {
    const item = document.createElement('div');
    item.innerHTML = `<span>${movie.title}</span>`
    dropdownMenuWrapper.appendChild(item);
  })
